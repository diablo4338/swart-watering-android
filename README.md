...
and non-publishing manual runs build the debug APK as a CI check. A manual run builds
and publishes a signed release instead when its `publish` input is enabled; the input
defaults to `false`.
